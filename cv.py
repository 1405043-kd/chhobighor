...
def white_balance_with_ancuti(img):
    ...
    result = cv2.cvtColor(img_red_compensated, cv2.COLOR_BGR2LAB)
...
def white_balance_without_ancuti(img):
    #img_red_compensated = ancuti_journal(img)
...
final = np.hstack((img, ancuti_journal(img), white_balance_with_ancuti(img), white_balance_without_ancuti(img)))
